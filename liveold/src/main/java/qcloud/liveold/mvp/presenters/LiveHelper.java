...
public class LiveHelper extends Presenter {
    ...
    public void setCameraPreviewChangeCallback() {
        System.out.println("----------weixin setCameraPre=" + QavsdkControl.getInstance().getAVContext());
...
//
            long elementCount = currMsg.getElementCount();
...
//        sendToUIThread(REFRESH_TEXT, textElem.getText(), sendId);
     ...
     * @param elem
     */
    private void handleCustomMsg(TIMElem elem, String identifier, String nickname, String headImg) {
        try {
            ...
            if (!customText.startsWith("{")) {
                ...
                int cmd = Integer.parseInt(split[1]);
                ...
                switch (cmd) {
                    case 2:  //老版本进入
                        if (mLiveView != null) {
                            mLiveView.memberJoin(identifier, nickname, headImg);
                        }
                        break;
                    case 3:  //老版本退出房间
                        if (mLiveView != null)
